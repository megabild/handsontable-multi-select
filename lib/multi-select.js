...
const getKeyValSeparator = R.path(["config", "separatorKeyVal"]);
...
export class MultiSelectEditor extends TextEditor {
  ...
  getValue() {
    ...
    return this.choices
      .getValue()
      .map((x) => x.value + this.separatorKeyVal + x.label)
      .join(this.separator);
  }
  ...
  prepare(row, col, prop, td, originalValue, cellProperties) {
    ...
    if (type === "numeric") {
      ...
      this.separatorKeyVal = getKeyValSeparator(this.selectOptions) || "\t";
  ...
  open() {
    ...
    const getOptions = () => {
      ...
      return Promise.resolve(toResolve).then((availableOptions) => {
        ...
        const selectedValues = originalValue.split(this.separator).map(x => x.split(this.separatorKeyVal)[0]);
        return R.map((item) => {
          const label = `${R.prop(this.valueKey, item)}`;

          return R.any(R.identical(label), selectedValues) ? R.assoc("selected", true, item) : item;
        }, availableOptions);
      });
    };
...
export function MultiSelectRenderer(instance, td, row, col, prop, value, cellProperties) {
  const { config = {}, options: availableOptions } = cellProperties.select;
